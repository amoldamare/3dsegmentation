...
class Model(object):
    ...
    def decoder(self, net, f1, f2, f3, f4, is_training=True, reuse=True):
        ...
        with slim.arg_scope([slim.conv2d],
            ...
            net = deconv_layer(net, [2, 2, start_channel, start_channel],
                               [batch_size, out_h * 2, out_w * 2, start_channel], 2, "up1", reuse=reuse)
            ...
            _, out_h, out_w, _ = net.get_shape().as_list()
            net = deconv_layer(net, [2, 2, start_channel, start_channel],
                               [batch_size, out_h * 2, out_w * 2, start_channel], 2, "up2", reuse=reuse)
            ...
            _, out_h, out_w, _ = net.get_shape().as_list()
            net = deconv_layer(net, [2, 2, start_channel, start_channel],
                               [batch_size, out_h * 2, out_w * 2, start_channel], 2, "up3", reuse=reuse)
            ...
            _, out_h, out_w, _ = net.get_shape().as_list()
            net = deconv_layer(net, [2, 2, start_channel, start_channel],
                               [batch_size, out_h * 2, out_w * 2, start_channel], 2, "up4", reuse=reuse)
    ...
    def net(self, inputs, is_training=True):
            ...
            for j in range(5):
                with tf.variable_scope("MRF"):
                    if j == 0:
                        ...
                        modality_fused1.append(tf.squeeze(
                            self.conv_fuse(concat_classifier, is_training=is_training, channel=start_channel,
                                           reuse=(i > 0 or j > 0)), axis=[1]))
                    elif j == 1:
                        concat_classifier = tf.transpose(tf.stack(fuse_feature2[i]), [1, 0, 2, 3, 4])
                        modality_fused2.append(tf.squeeze(
                            self.conv_fuse(concat_classifier, is_training=is_training, channel=start_channel,
                                           reuse=(i > 0 or j > 0)), axis=[1]))
                    elif j == 2:
                        concat_classifier = tf.transpose(tf.stack(fuse_feature3[i]), [1, 0, 2, 3, 4])
                        modality_fused3.append(tf.squeeze(
                            self.conv_fuse(concat_classifier, is_training=is_training, channel=start_channel,
                                           reuse=(i > 0 or j > 0)), axis=[1]))
                    elif j == 3:
                        concat_classifier = tf.transpose(tf.stack(fuse_feature4[i]), [1, 0, 2, 3, 4])
                        modality_fused4.append(tf.squeeze(
                            self.conv_fuse(concat_classifier, is_training=is_training, channel=start_channel,
                                           reuse=(i > 0 or j > 0)), axis=[1]))
                    elif j == 4:
                        concat_classifier = tf.transpose(tf.stack(mod[i]), [1, 0, 2, 3, 4])
                        modality_fused.append(tf.squeeze(
                            self.conv_fuse(concat_classifier, is_training=is_training, channel=start_channel,
                                           reuse=(i > 0 or j > 0)), axis=[1]))

        f_shape = modality_fused[0].get_shape().as_list()
        cell = convLSTM.ConvLSTMCell(start_channel, k_size=3, height=f_shape[1], width=f_shape[2],
        ...
        for i in range(self.params.sequence_length):
            with tf.variable_scope("Decoder"):
                decode_feature = self.decoder(output[i], modality_fused1[i], modality_fused2[i], modality_fused3[i],
                                              modality_fused4[i], is_training=is_training, reuse=(i > 0))
            with slim.arg_scope([slim.conv2d],
                                activation_fn=tf.nn.relu,
                                #normalizer_fn=slim.batch_norm,
                                #normalizer_params={'is_training': is_training},
                ...
                conv_classifier = slim.conv2d(decode_feature, self.params.num_classes, [1, 1], activation_fn=None,
                                              scope='classify')
    ...
    def weighted_losses_without_iou(self, logits, labels):
        """
        loss_weight = np.array([
            0.10455609709637404,
            1.0,
