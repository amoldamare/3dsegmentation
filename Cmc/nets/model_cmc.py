...
rand_init_var = 0.05  # 0.1 was good
l2_reg_val = 0.0005 #was 0.0005
bnorm_decay = 0.999 # 0.1 was good
...
def DTS(X, r, name):
    ...
    with tf.variable_scope(name):
        with slim.arg_scope([slim.conv2d],
                                activation_fn=None,
                                weights_initializer=tf.truncated_normal_initializer(0.0, rand_init_var),
                                weights_regularizer=slim.l2_regularizer(l2_reg_val)):
...
class Model(object):
    ...
    def conv_fuse(self, net, is_training=True, channel=20, reuse=False):
        with tf.variable_scope('CMC2', reuse=reuse) as scope:
            kernel3D_2 = _variable_with_weight_decay('weights_3d',
                                            ...
                                            initializer=tf.truncated_normal_initializer(0.0, rand_init_var),
                                            wd=None)
    ...
    def encoder(self, inputs, is_training=True, reuse=False):
        ...
        with slim.arg_scope([slim.conv2d],
                            ...
                            normalizer_params={'is_training': is_training, 'decay':bnorm_decay,'zero_debias_moving_mean':True,'scale':True},
                            weights_initializer=tf.truncated_normal_initializer(0.0, rand_init_var),
                            weights_regularizer=None, #slim.l2_regularizer(l2_reg_val),
                            reuse=reuse):
    ...
    def decoder(self, net, f1, f2, f3, f4, is_training=True, reuse=True):
        ...
        with slim.arg_scope([slim.conv2d],
                            activation_fn=tf.nn.relu,
                            normalizer_fn=None, #slim.batch_norm,
                            normalizer_params={'is_training': is_training, 'decay':bnorm_decay, 'center':False, 'zero_debias_moving_mean':True,'scale':True},
                            weights_initializer=tf.truncated_normal_initializer(0.0, rand_init_var),
                            weights_regularizer=None,#slim.l2_regularizer(l2_reg_val),
                            reuse=reuse):
    ...
    def net(self, inputs, is_training=True):
        ...
        for i in range(self.params.sequence_length):
            ...
            with slim.arg_scope([slim.conv2d],
                            ...
                            weights_regularizer=None, #slim.l2_regularizer(l2_reg_val),
                            reuse=(i>0)):
                #decode_feature = slim.dropout(decode_feature, 0.8, is_training=is_training)
...
#            1.0
#        ])
        #loss_weight = np.ones((2),dtype=float)

        loss_weight = np.array([1,0.5])
        ...
        for logit, target in zip(logits, labels):
            ...
            # streaming iou have been replaced with conventional one

            #mean_iou = slim.metrics.streaming_mean_iou(tf.reshape(tf.argmax(softmax, 1), (-1,1)), labels_for_eval, self.params.num_classes)
            mean_iou = tf.metrics.mean_iou(tf.reshape(tf.argmax(softmax, 1), (-1, 1)), labels_for_eval,
    ...
    def weighted_losses_growth_term(self, logits, labels):
        l = 0.00 # was -0.05
        ...
        for logit, target in zip(logits, labels):
            ...
            softmax = tf.nn.softmax(logit)
            mean_iou = slim.metrics.streaming_mean_iou(tf.reshape(tf.argmax(softmax, 1), (-1, 1)), labels_for_eval,
                                                       self.params.num_classes)
            cross_entropy = -tf.reduce_sum(tf.multiply(target * tf.log(softmax + epsilon), loss_weight), axis=[1])
            softmax_log.append(softmax)
            target_log.append(target)

            # cross_entropy = self.generalised_dice_loss(logit, target)
            loss_list.append(cross_entropy*(1))
            i += 1
        growth_loss_list = []
        for i in range(1,len(softmax_log)):
            growth_loss = 1/(1 + tf.exp((softmax_log[i-1] - softmax_log[i])*10)) - 0.5

            past = softmax_log[i-1]
            current = softmax_log[i]

            current_val = 0
            for j in range(2):
                nom = tf.reduce_sum(tf.multiply(past[j],current[j]))
                denom = tf.reduce_sum(past[j]) + tf.reduce_sum(current[j]) - tf.reduce_sum(tf.multiply(past[j],current[j]))

                current_val += tf.divide(nom,denom)

            growth_loss = current_val/2



            #growth_loss = softmax_log[i] - softmax_log[i-1]
            #growth_loss_list.append(tf.reduce_sum(growth_loss[:,1]))
            loss_list[i] = loss_list[i] + l * growth_loss #l * growth_loss[:,1]
        ...
        loss = tf.add_n(tf.get_collection('losses'), name='total_loss')
        tf.losses.add_loss(loss)
        return loss, mean_iou, loss_list
